...
export function getLastLezioniVuoteDebug(): { error: string | null; count: number } {
  return { error: lastLezioniVuoteError, count: lastLezioniVuoteCount }
}
 ...
 * Usato quando il "Totale" dell'abbonamento non coincide con quanto gi� pagato (es. acconto + resto in rate).
 */
...
export interface CrmAppuntamentoRow {
  ...
  /** DestinatarioNomeOperatore nel gestionale (consulente assegnata). */
...
async function crmSelectExtraFragments(view: string): Promise<{ select: string; map: (row: Record<string, unknown>) => Partial<CrmAppuntamentoRow> }> {
  ...
  const destCol = await crmPickFirstCol(view, ["DestinatarioNomeOperatore", "Destinatario", "NomeDestinatario"])
  ...
  if (destCol) selectParts.push(`${destCol} AS ConsulenteNome`)
...
/** Destinatario CRM = consulente (match parziale: «Ombretta Zenoni» ↔ «Ombretta»). */
async function crmDestinatarioMatchSql(view: string): Promise<string> {
  const col =
    (await crmPickFirstCol(view, ["DestinatarioNomeOperatore", "Destinatario", "NomeDestinatario"])) ??
    "DestinatarioNomeOperatore"
  return `(
    LOWER(LTRIM(RTRIM(COALESCE(${col}, N'')))) = LOWER(LTRIM(RTRIM(COALESCE(@nomeOperatore, N''))))
    OR LOWER(LTRIM(RTRIM(COALESCE(@nomeOperatore, N'')))) LIKE LOWER(LTRIM(RTRIM(COALESCE(${col}, N'')))) + N'%'
    OR LOWER(LTRIM(RTRIM(COALESCE(${col}, N'')))) LIKE LOWER(LTRIM(RTRIM(COALESCE(@nomeOperatore, N'')))) + N'%'
    OR CHARINDEX(LOWER(LTRIM(RTRIM(COALESCE(${col}, N'')))), LOWER(LTRIM(RTRIM(COALESCE(@nomeOperatore, N''))))) > 0
    OR CHARINDEX(LOWER(LTRIM(RTRIM(COALESCE(@nomeOperatore, N'')))), LOWER(LTRIM(RTRIM(COALESCE(${col}, N''))))) > 0
  )`
}

/** Evase: filtro su DataEvasione; da fare: su DataAppuntamento (come gestionale storico). */
async function crmTelefonateDateRangeSql(view: string): Promise<string> {
  const evasoCol = await crmPickFirstCol(view, ["DataEvasione", "DataEvaso", "EvasoIl", "DataEvasioneStorico"])
  if (!evasoCol) {
    return `CAST(DataAppuntamento AS DATE) >= CAST(@from AS DATE) AND CAST(DataAppuntamento AS DATE) <= CAST(@to AS DATE)`
  }
  return `(
    (
      ${evasoCol} IS NOT NULL
      AND CAST(${evasoCol} AS DATE) >= CAST(@from AS DATE)
      AND CAST(${evasoCol} AS DATE) <= CAST(@to AS DATE)
    )
    OR (
      ${evasoCol} IS NULL
...
/** Storico CRM telefonate (3. Telefonica, Destinatario = consulente, range su evasione/appuntamento). */
export async function queryCrmTelefonateOperatore(params: {
  ...
  try {
    ...
    const destinatarioWhere = await crmDestinatarioMatchSql(view)
    const dateRangeWhere = await crmTelefonateDateRangeSql(view)
 ...
 * - Tesseramento gare: Macro 5 + Cat 19 + Abb 401 + Durata 5363 (FIN+Iscrizione / TESSERAMENTO GARE)
 */
  ...
  return `
    LogsFromView AS (
      ...
      WHERE CAST(L.[DataOperazione] AS DATE) >= CAST(${args.fromParam} AS DATE)
        ...
        AND ${sqlWhereLogCrossEvent("L.[AppLogDescrizione]")}
    ...
    LogsResolved AS (
      ...
      FROM LogsFromView V
      OUTER APPLY (
        SELECT TOP 1 ${parseDesc("L2.[AppLogDescrizione]")} AS IDIscrizione
        FROM ${args.logViewQuery} L2
        WHERE V.IDUtente IS NOT NULL
          AND TRY_CONVERT(int, L2.[IDUtente]) = V.IDUtente
          AND CAST(L2.[DataOperazione] AS DATE) = CAST(V.LogData AS DATE)
          AND L2.[AppLogDescrizione] LIKE N'%IDIscrizione =%'
          AND ${parseDesc("L2.[AppLogDescrizione]")} IS NOT NULL
        ORDER BY L2.[DataOperazione] DESC
      ) IdDay
      OUTER APPLY (
...
function sqlUnionLegacyAppLogCross(fromParam: string, toParam: string): string {
  ...
  return `
      ...
      WHERE CAST(L.[DataOperazione] AS DATE) >= CAST(${fromParam} AS DATE)
        ...
        AND ${sqlWhereLogCrossEvent("L.[Descrizione]")}`
}

type PagamentiCrossSqlParts = {
  pvq: string
  idExpr: string
  importoExpr: string
  paidDateExpr: string
...
async function getPagamentiCrossSqlParts(): Promise<PagamentiCrossSqlParts | null> {
  ...
  let colsLower: string[]
  try {
    colsLower = await prenGetCols(pagView)
  } catch {
    return null
  }
  const idCol =
  ...
  const importoCol =
    pickBestNumberCol(colsLower, ["AbbonamentiPagamentiImporto", "Abbonamenti Pagamenti Importo", "ImportoRata", "Importo"]) ??
    pickBestTextCol(colsLower, ["AbbonamentiPagamentiImporto", "Abbonamenti Pagamenti Importo", "ImportoRata", "Importo"])
  const rataDateCol = pickBestDateCol(colsLower, [
    "AbbonamentiPagamentiDataRata",
    "Abbonamenti Pagamenti Data Rata",
    "DataRata",
    "Data Rata",
  ])
  const paidDateCol = pickPaidDateColStrict(colsLower)
  if (!idCol || !importoCol) return null
  return {
    pvq,
    idExpr: `TRY_CONVERT(int, P.${bracketCol(idCol)})`,
    importoExpr: `TRY_CONVERT(float, P.${bracketCol(importoCol)})`,
    paidDateExpr: paidDateCol ? `TRY_CONVERT(datetime, P.${bracketCol(paidDateCol)})` : "NULL",
    rataDateExpr: rataDateCol ? `TRY_CONVERT(datetime, P.${bracketCol(rataDateCol)})` : "NULL",
  }
}

/** Totale cross nel periodo: log (Cross selling / cambio tipo) → SUM(AbbonamentiPagamentiImporto) per IDIscrizione. */
export async function queryVenditeCrossTotaleDaLogPagamenti(
  ...
  return rows.reduce((s, r) => s + r.totale, 0)
}
    ...
    const r = await req.query(
      `;WITH LF AS (
        ...
        WHERE CAST(L.[DataOperazione] AS DATE) >= CAST(@from AS DATE)
          ...
          -- Passaggi verso abbonamento CROSS (varianti testuali sul log gestionale).
          AND (
            UPPER(L.[AppLogDescrizione]) LIKE N'%CROSS%'
            OR UPPER(L.[AppLogDescrizione]) LIKE N'%ABBONAMENTO CROSS%'
            OR UPPER(L.[AppLogDescrizione]) LIKE N'%TIPO%ABBONAMENTO%CROSS%'
          )
...
type SqlColIndex = { lower: Set<string>; exact: Map<string, string> }
const sqlColsCache = new Map<string, SqlColIndex>()

async function sqlObjectCols(objName: string): Promise<SqlColIndex> {
  const clean = qualifySqlObject(objName).objectId
  const cached = sqlColsCache.get(clean)
  if (cached) return cached
  const pool = await getPool()
  const empty: SqlColIndex = { lower: new Set(), exact: new Map() }
  if (!pool) return empty
  try {
    const r = await pool.request().input("obj", sql.NVarChar, clean).query(
    ...
    for (const row of (r.recordset ?? []) as { name?: string }[]) {
      const name = String(row?.name ?? "").trim()
      if (!name) continue
      lower.add(name.toLowerCase())
      exact.set(name.toLowerCase(), name)
    }
    const out = { lower, exact }
    sqlColsCache.set(clean, out)
    return out
  } catch {
    return empty
  }
...
function pickSqlCol(idx: SqlColIndex, candidates: string[]): string | null {
  for (const c of candidates) {
    const ex = idx.exact.get(c.toLowerCase())
    if (ex) return ex
  }
  return null
}

function lessonIdFromPrenRow(r: PrenotazioneCorsoRow): number | null {
  const raw = (r.raw ?? {}) as Record<string, unknown>
  const v =
    firstNonEmpty(raw, ["IDPrenotazioneLezione", "IdPrenotazioneLezione", "IDLezione", "LezioneId"]) ?? undefined
  const n = v != null ? Number(v) : NaN
  return Number.isFinite(n) && n > 0 ? n : null
}

function buildWeekdaySqlForAlias(idx: SqlColIndex, giornoParam: string, alias: string): string[] {
  const parts: string[] = []
  const dayFlags: { cols: string[]; dow: number }[] = [
  ...
  for (const d of dayFlags) {
    const col = pickSqlCol(idx, d.cols)
    if (!col) continue
    parts.push(
      `(DATEPART(weekday, CAST(${giornoParam} AS date)) = ${d.dow} AND TRY_CAST(${alias}.[${col}] AS int) NOT IN (0) AND ${alias}.[${col}] IS NOT NULL)`
    )
  }
  const gs = pickSqlCol(idx, ["GiornoSettimana", "GGWeek", "Weekday", "WeekDay", "IDGiorno", "Giorno", "Giorni"])
  if (gs) {
    parts.push(`TRY_CAST(${alias}.[${gs}] AS int) = DATEPART(weekday, CAST(${giornoParam} AS date))`)
  }
  return parts
}
...
function buildPrenotazioniWeekdaySql(prenIdx: SqlColIndex, plIdx: SqlColIndex, giornoParam: string): string | null {
  const parts = [
    ...buildWeekdaySqlForAlias(prenIdx, giornoParam, "p"),
    ...buildWeekdaySqlForAlias(plIdx, giornoParam, "pl"),
  ]
  if (parts.length === 0) return null
  return `(${parts.join(" OR ")})`
}

function buildPrenotazioniDateRangeSql(prenIdx: SqlColIndex, giornoParam: string, alias = "p"): string | null {
  const di = pickSqlCol(prenIdx, ["DataInizio", "Datainizio", "DataDa", "InizioValidita"])
  const df = pickSqlCol(prenIdx, ["DataFine", "Datafine", "DataA", "FineValidita"])
  if (di && df) {
    return `(CAST(${alias}.[${di}] AS date) <= CAST(${giornoParam} AS date) AND (${alias}.[${df}] IS NULL OR CAST(${alias}.[${df}] AS date) >= CAST(${giornoParam} AS date)))`
  }
  if (di) return `(CAST(${alias}.[${di}] AS date) <= CAST(${giornoParam} AS date))`
  return null
}
...
async function queryLezioniCorsiSenzaIscritti(
  giorno: string,
  base: PrenotazioneCorsoRow[],
  enrich: (raw: Record<string, unknown>) => PrenotazioneCorsoRow
): Promise<PrenotazioneCorsoRow[]> {
  lastLezioniVuoteError = null
  lastLezioniVuoteCount = 0
  if (process.env.GESTIONALE_CORSI_INCLUDE_LEZIONI_VUOTE === "false") return []
  const pool = await getPool()
  if (!pool) {
    lastLezioniVuoteError = "pool SQL non disponibile"
    return []
  }

  const seenLez = new Set<number>()
  for (const r of base) {
    const id = lessonIdFromPrenRow(r)
    if (id) seenLez.add(id)
  }

  const rawPl = (process.env.GESTIONALE_TABLE_PRENOTAZIONI_LEZIONI ?? "dbo.PrenotazioniLezioni").trim()
  const rawPren = (process.env.GESTIONALE_TABLE_PRENOTAZIONI ?? "dbo.Prenotazioni").trim()
  if (!isSafeSqlIdentifierLoose(rawPl) || !isSafeSqlIdentifierLoose(rawPren)) {
    lastLezioniVuoteError = "nome tabella PrenotazioniLezioni/Prenotazioni non valido"
    return []
  }
  ...
  const mapRows = (recordset: Record<string, unknown>[] | undefined): PrenotazioneCorsoRow[] => {
    ...
    for (const raw0 of recordset ?? []) {
      ...
      const raw: Record<string, unknown> = {
        ...
        DataInizioPrenotazioneIscrizione: `${giorno}T00:00:00`,
      }
...
export async function queryPrenotazioniCorsi(params?: { giorno?: string }): Promise<PrenotazioneCorsoRow[]> {
  ...
  try {
    ...
    if (!(giornoOk && !dateCol)) {
      ...
      const rows = (r.recordset ?? []) as Record<string, unknown>[]
      base = rows.map((raw) => enrich(raw))
    } else {
      lastPrenotazioniQueryError =
        "Colonna data non trovata nella vista prenotazioni; elencate solo lezioni senza iscritti"
    }
    if (giornoOk && giorno) {
      const vuote = await queryLezioniCorsiSenzaIscritti(giorno, base, enrich)
